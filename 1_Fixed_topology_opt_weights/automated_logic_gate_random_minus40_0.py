...
import importlib
import spatial_logic_gate_QO_KY_automate_random_minus40_0 as slg
###############################################################################
# Clear the terminal
...
for ii in range(1):
    ...
    np.save(f'logic_gate_x_indices_{logic_gates[ii]}_weights_40_0.npy', best_x_indices_opt)
    np.save(f'logic_gate_y_indices_{logic_gates[ii]}_weights_40_0.npy', best_y_indices_opt)
    ###############################################################################
    #         Generating predicted output from the optimised weights
    ###############################################################################
    log_w = slg.calculate_weights(best_x_indices_opt, best_y_indices_opt)
    np.save(f'logic_gate_weights_{logic_gates[ii]}_weights_40_0.npy', log_w)
    ...
    np.save(f'logic_gate_output_sql_{logic_gates[ii]}_weights_40_0.npy', logic_data)
    ###############################################################################
    ...
    #plt.show()
    np.save(f'output_values_{logic_gates[ii]}_weights_40_0.npy', np.array(values))
   ###############################################################################
  
plt.figure(1)
plt.savefig("Training_actual_sq_lattice_weights_40_0.png")

plt.figure(2)
plt.savefig("Predicted_data_actual_sq_lattice_weights_40_0.png")

plt.figure(3) 
plt.savefig("Output_ON_OFF_actual_sq_lattice_weights_40_0.png")