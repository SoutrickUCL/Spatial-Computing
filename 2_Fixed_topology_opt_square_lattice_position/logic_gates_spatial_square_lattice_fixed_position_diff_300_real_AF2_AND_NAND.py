...
import importlib
import spatial_logic_gate_fixed_positions_diff_300_real_AF2_AND_NAND as slg
###############################################################################
# Clear the terminal
...
###############################################################################
gen_func=np.array([slg.generate_AND,
       slg.generate_NAND])
###############################################################################
...
hidden_layer_config=[ hidden_nodes_AND, hidden_nodes_NAND]
output_layer_config=[output_node_AND, output_node_NAND ]
###################################################################################
...
#for ii in range(1):
    ...
    np.save(f'logic_gate_x_indices_{logic_gates[ii]}_fixed_position_diff_300_REAL_AF2_only_AND_NAND.npy', best_x_indices_opt)
    np.save(f'logic_gate_y_indices_{logic_gates[ii]}_fixed_position_diff_300_REAL_AF2_only_AND_NAND.npy', best_y_indices_opt)
    ###############################################################################
    #         Generating predicted output from the optimised weights
    ###############################################################################
    log_w = slg.calculate_weights(best_x_indices_opt, best_y_indices_opt)
    np.save(f'logic_gate_weights_{logic_gates[ii]}_fixed_position_diff_300_REAL_AF2_only_AND_NAND.npy', log_w)
    ...
    np.save(f'logic_gate_output_sql_{logic_gates[ii]}_fixed_position_diff_300_REAL_AF2_only_AND_NAND.npy', logic_data)
    ###############################################################################
    ...
    #plt.show()
    np.save(f'output_values_{logic_gates[ii]}_fixed_position_diff_300_REAL_AF2_only_AND_NAND.npy', np.array(values))
   ###############################################################################
  
plt.figure(1)
plt.savefig("Training_actual_sq_lattice_fixed_position_diff_300_REAL_AF2_only_AND_NAND.png")

plt.figure(2)
plt.savefig("Predicted_data_actual_sq_lattice_fixed_position_diff_300_REAL_AF2_only_AND_NAND.png")

plt.figure(3) 
plt.savefig("Output_ON_OFF_actual_sq_lattice_fixed_position_diff_300_REAL_AF2_only_AND_NAND.png")