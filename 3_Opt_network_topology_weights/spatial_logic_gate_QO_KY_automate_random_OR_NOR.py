...
def run_genetic_algorithm(X, Y, ndata):
    ...
    for ii in range(npop):
         ...
         weights[0][ii] = [random.uniform(-70,0) for _ in range(3*num_hidden_nodes)] # Initialize the weights with random values
    #print(weights[0])
    #------------------------------------------------------------------------
    ...
    for ii in range(1,ngen): 
        ...
        for jj in range(pop_cut,npop):
            if(len(weights[ii][jj]) != (len(network_topology[ii][jj][:])-1)*3):
                    weights[ii][jj] = [random.uniform(-70,0) for _ in range((len(network_topology[ii][jj][:])-1)*3)]
        #-------------------------------------------------------------------------------------------------
        #                                   Recalculate fitness
        #-------------------------------------------------------------------------------------------------
        for jj in range(npop):
