...
import importlib
import spatial_logic_gate_QO_KY_automate_topology_position_AND_NAND_Chris_prob_max3 as slg
# Clear the terminal
...
#for ii in range(len(logic_gates)):
    ...
    ###############################################################################
    log_w, best_network_topology, best_x_indices_opt, best_y_indices_opt = slg.run_genetic_algorithm(X, Y, ndata)
    print(best_network_topology,log_w)
    pref_network_topology[ii] = best_network_topology
    np.save(f'logic_gate_x_indices_{logic_gates[ii]}_predicted_opt_loc_topo_prob_m3.npy', best_x_indices_opt)
    np.save(f'logic_gate_y_indices_{logic_gates[ii]}_predicted_opt_loc_topo_prob_m3.npy', best_y_indices_opt)
    ###################################################################################
    ...
    np.save(f'logic_gate_output_{logic_gates[ii]}_predicted_opt_loc_topo_prob_m3.npy', logic_data)
    ###################################################################################
    ...
    for jj in range(4):
      ...
      print(logic_inputs[jj], np.log10(final_output))
    categories = ['00', '10', '01', '11']
    values = np.log10(data_output)
    ...
    ###############################################################################
    ANDmalized_values = (values - np.min(values)) / (np.max(values) - np.min(values))
    colors = plt.cm.viridis(ANDmalized_values)
    plt.figure(3)
    plt.rcParams['axes.spines.top'] = False
    plt.rcParams['axes.spines.right'] = False
    plt.subplot(2,3, ii+1)
    ...
    np.save(f'logic_data_output_predicted_opt_loc_topo_{logic_gates[ii]}_prob_m3.npy', logic_data)
    np.save(f'logic_data_weights_predicted_opt_loc_topo_{logic_gates[ii]}_prob_m3.npy', log_w)
...
plt.figure(1)
plt.savefig("Training_automate_opt_loc_topo_NAND_prob_m3.png")

plt.figure(2)
plt.savefig("Predicted_automate_opt_loc_topo_NAND_prob_m3.png")

plt.figure(3) 
plt.savefig("Output_automate_predicted_opt_loc_topo_NAND_prob_m3.png")

with open('pref_network_topology_predicted_opt_loc_topo_NAND_prob_m3.txt', 'w') as f:
