...
def run_genetic_algorithm(X, Y, ndata):
    ...
    #Initialing locations for two input nodes, two hidden nodes and one output node
        x_indices_pop_trail = np.array([0, 0, 0, 0, 3800])
        ...
        # iteration = 0
        while True:
            x_indices_pop_trail[0] = np.random.uniform(100, 3600)  # X index of input 1
            x_indices_pop_trail[1] = np.random.uniform(100, 3600)  # X index of input 2 
            x_indices_pop_trail[2] = np.random.uniform(100, 3600)  # X index of hidden node 1
            x_indices_pop_trail[3] = np.random.uniform(100, 3600)  # X index of hidden node 2

            y_indices_pop_trail[0] = np.random.uniform(100, 3600)  # Y index of input node 1
            y_indices_pop_trail[1] = np.random.uniform(100, 3600)  # Y index of input node 2
            y_indices_pop_trail[2] = np.random.uniform(100, 3600)  # Y index of hidden node 1
            y_indices_pop_trail[3] = np.random.uniform(100, 3600)  # Y index of hidden node 2
            ...
            if len(x_indices_pop[ii][jj])!= num_hidden_nodes+3:
                ...
                if n_add_delete > 0:
                    for _ in range(n_add_delete):
                        ...
                        new_x_index = np.random.randint(100, 3600)
                        new_y_index = np.random.randint(100, 3600)
