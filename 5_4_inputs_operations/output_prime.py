...
import importlib
import spatial_prime as slg
import csv
# Clear the terminal
os.system('cls' if os.name == 'nt' else 'clear')
###############################################################################
logic_gates=['Prime', 'PerfectPower', 'Vowel']
###############################################################################
#       Functions for generating Traning Data for different logic gates
###############################################################################
gen_func=np.array([slg.generate_Prime,
                   slg.generate_PerfectPower,
                   slg.generate_Vowel])
###################################################################################
...
for ii in range(len(logic_gates)):
    ...
    ###############################################################################
    Y = gen_func[ii](X2,on_cutoff, off_cutoff) # Generates the training output data
    #print(X2,Y)
    ###############################################################################
    ...
    Xtest=X2
    #print(Xtest)
    Ytest = gen_func[ii](X2, on_cutoff, off_cutoff)
    YY = np.zeros([ntest])
    ...
    if ii == 2:
      plt.xticks(x, [chr(65 + i) for i in range(len(x))], fontsize=10)  
       ...
       plt.xticks(np.round(x).astype(int), np.round(x).astype(int), fontsize=10) 
