...
def run_genetic_algorithm(X, Y, ndata):
    ...
    for ii in range(npop):
         ...
         weights[0][ii] = [random.uniform(-50,1) for _ in range(5*num_hidden_nodes)] # Initialize the weights with random values
    ...
    for ii in range(1,ngen): 
        ...
        for jj in range(pop_cut,npop):
            if(len(weights[ii][jj]) != (len(network_topology[ii][jj][:])-1)*5):
                    weights[ii][jj] = [random.uniform(-50,1) for _ in range((len(network_topology[ii][jj][:])-1)*5)]
        #-------------------------------------------------------------------------------------------------
        #                                   Recalculate fitness
        #-------------------------------------------------------------------------------------------------
        for jj in range(pop_cut,npop):
