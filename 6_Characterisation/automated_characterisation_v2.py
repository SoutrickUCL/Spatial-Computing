...
import importlib
import spatial_characterisation_v2 as slg
...
for ii in range(1,2):
    ...
    if (noutput2=='HP'):
        cutoff = 10**2.57
        cuton = 10**6.13
        print(np.max(logic_data[:,3]), np.min(logic_data[:,3]),noutput2, cutoff, cuton)
    ...
    z2 =(logic_data[:,3]-cutoff)/(cuton-cutoff)
    z3= (logic_data[:,3])
    ...
    plt.rcParams['axes.spines.right'] = False
    plt.subplot(2,2, ii+1)
    plt.scatter(x, y, c=z3, cmap='viridis', s=15, edgecolors='none')
    cbar = plt.colorbar()
    plt.xlabel('X1')
    plt.ylabel('X2')
    plt.title(analog[ii]) 
...
#     np.save(f'logic_data_output_analog_{analog[ii]}.npy', logic_data)
#     np.save(f'logic_data_weights_analog__{analog[ii]}.npy', log_w)
###############################################################################
  
plt.figure(1)
plt.savefig("Training_automate_analog.png")

plt.figure(2)
plt.savefig("Predicted_automate_analog.png")

plt.figure(3)
plt.savefig("Predicted_automate_analog_test.png")

# plt.figure(3) 
# plt.savefig("Output_automate_analog.png")

with open('pref_network_topology_circular.txt', 'w') as f:
