...
def generate_circular(X):
    ...
    # Replace 1 with cuton and 0 with cutoff
    Y = np.where(dist_sq <= radius_log**2, cuton, cutoff)
    return Y
#----------------------------------------------------------------------
def generate_spiral(X):
    ...
    # Replace 1 with cuton and 0 with cutoff
    Y = np.where((radius - theta * num_spirals / (2 * np.pi)) % 2 < 1, cuton, cutoff)
    return Y
#------------------------------------------------------------------------
def generate_checked_box(X):
    ...
    # Replace 1 with cuton and 0 with cutoff
    Y = np.where(((x_indices + y_indices) % 2) == 0, cutoff, cuton)
    return Y
...
def run_genetic_algorithm(X, Y, ndata):
    ...
    npop = 100000 # Number of network topologies
